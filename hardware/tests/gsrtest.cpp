...
// Artificial GSR generation for demonstration
const float GSR_BASELINE = 300.0;  // Typical healthy baseline value
unsigned long gsrStartTime = 0;     // For time-based patterns
...
void setup() {
  ...
  gsrStartTime = millis();
  randomSeed(analogRead(0));  // Initialize random seed for GSR noise
...
void loop() {
  ...
  if (!calibrationComplete) {
    if (currentMillis - calibrationStartTime < 5000) {
      if (currentMillis - lastGSRSampleTime >= 20) {
        // Use artificial baseline for demonstration (instead of reading sensor)
        rawGSR = (int)GSR_BASELINE;
  ...
  if (!devicePoweredOff) {
    ...
    if (calibrationComplete) {
      ...
      // Haptic feedback for high stress (>80%) - use display value
      digitalWrite(VIBRO_MOTOR_PIN, (stressIndexDisplay > 80) ? HIGH : LOW);
 ...
 * Generates artificial healthy GSR fluctuations for demonstration purposes.
 * Creates realistic patterns with slow circadian variations and short-term fluctuations.
 */
void updateGSR() {
  if (!calibrationComplete) return;
  
  unsigned long currentMillis = millis();
  float elapsedSeconds = (currentMillis - gsrStartTime) / 1000.0;
  
  // Generate artificial GSR with healthy fluctuations
  // Base value with multiple sine waves for realistic variation
  
  // Slow wave (circadian rhythm) - period ~2 hours
  float slowWave = sin(elapsedSeconds / 7200.0 * 2.0 * PI) * 15.0;
  
  // Medium wave (breathing/relaxation cycles) - period ~30 seconds
  float mediumWave = sin(elapsedSeconds / 30.0 * 2.0 * PI) * 8.0;
  
  // Fast wave (short-term variations) - period ~5 seconds
  float fastWave = sin(elapsedSeconds / 5.0 * 2.0 * PI) * 5.0;
  
  // Small random noise for realism (±2)
  float noise = (random(-200, 200) / 100.0);
  
  // Combine all components
  float artificialGSR = GSR_BASELINE + slowWave + mediumWave + fastWave + noise;
  
  // Constrain to reasonable range (150-500 ADC typical for healthy GSR)
  artificialGSR = constrain(artificialGSR, 150.0, 500.0);
  
  rawGSR = (int)artificialGSR;
 ...
 * User can wake by holding button for 10 seconds again.
 */
void enterPowerOff() {
...
void wakeFromPowerOff() {
  ...
  // Haptic feedback - 2 short pulses
  for (int i = 0; i < 2; i++) {
    digitalWrite(VIBRO_MOTOR_PIN, HIGH);
    delay(100);
    digitalWrite(VIBRO_MOTOR_PIN, LOW);
